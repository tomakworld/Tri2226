...
function gen(phase, wi, rec){
  ...
  if (phase==="base"){
    ...
    return { run:runOf(phase, Math.round((16+wi*2)*(rec?0.8:1)), rec), swim:{
      ...
      wed:{t:"Z2+Tempo收尾", x:`${bw}分 ${Z.Z2},最後15分 ${Z.Z3}(連續不休);跑步間歇在前,間隔4小時+`, v:`${bw}分`, tss:tss([[bw-15,0.65],[15,0.83]])},
      thu:{t:"閾值間歇", x:`熱身15分;主課 4x${bte}分 ${Z.Z3}上緣~${Z.Z4}下緣(可取中段93-95%FTP即${W(0.93)}-${W(0.95)}W,務必開風扇);息4分(約4:1,短休息練有氧天花板與乳酸排除);緩和10分`, v:`${15+4*bte+16+10}分`, tss:tss([[15,0.6],[4*bte,0.88],[16,0.5],[10,0.55]])},
  ...
  if (phase==="build1"){
    ...
    return { run:runOf(phase, Math.round((21+wi*2)*(rec?0.75:1)), rec), swim:{
      ...
      wed:{t:"Z2+Sweet Spot", x:`${bw}分 ${Z.Z2},中段2x15分 ${Z.SS},組間休5分 ${Z.Z2}(3:1);跑步間歇後4小時+`, v:`${bw}分`, tss:tss([[bw-30,0.65],[30,0.90]])},
      thu:{t:"FTP強化", x:`熱身15分;主課 3x${bte}分 ${Z.Z4}(室內${W(0.91)}W起);息5分(約3.5:1,短休息);緩和10分`, v:`${15+3*bte+15+10}分`, tss:tss([[15,0.6],[3*bte,0.92],[15,0.5],[10,0.55]])},
  ...
  if (phase==="build2"){
    ...
    return { run:runOf(phase, Math.min(Math.round((24+wi*2)*(rec?0.75:1)),30), rec), swim:{
      ...
      wed: wi===2
        ? {t:"VO2max 天花板課", x:`熱身20分 ${Z.Z2};主課 5x3分 @${W(1.10)}-${W(1.15)}W(110-115%FTP),組間休3分 ${Z.Z1}(1:1,完全恢復再出發);緩和15分。此為整期唯一一堂VO2max,目的是推高有氧天花板 — 天花板升高後,下方Sweet Spot與比賽功率的耐受度會跟著提升。跑步間歇後4小時+`, v:"70分", tss:78}
        : {t:"Z2+Sweet Spot", x:`${bw}分 ${Z.Z2},中段2x18分 ${Z.SS},組間休6分 ${Z.Z2}(3:1);跑步間歇後4小時+`, v:`${bw}分`, tss:tss([[bw-36,0.65],[36,0.90]])},
      thu:{t:"FTP高峰", x:`熱身15分;主課 3x${bte}分 ${Z.Z4} 下緣;息5分(約3.5:1,短休息);緩和10分`, v:`${15+3*bte+15+10}分`, tss:tss([[15,0.6],[3*bte,0.93],[15,0.5],[10,0.55]])},
  ...
  if (phase==="peak"){
    const T=[
      { run:runOf(phase,28,false), swim:{
        ...
        bike:{
          wed:{t:"Z2+Sweet Spot", x:`100分 ${Z.Z2},中段2x15分 ${Z.SS},組間休5分 ${Z.Z2}`, v:"100分", tss:92},
          thu:{t:"FTP高峰", x:`熱身15分;主課 3x20分 ${Z.Z4} 下緣;息5分(約3.5:1,短休息);緩和10分`, v:"110分", tss:105},
      ...
      { run:runOf(phase,20,false), swim:{
        ...
        bike:{
          ...
          thu:{t:"FTP維持", x:`熱身15分;主課 3x12分 ${Z.Z4};息4分(約4:1,短休息練有氧天花板與乳酸排除);緩和10分`, v:"75分", tss:71},
...
export default function SeasonPlan(){
  ...
  const rows = week.race ? [] : [
    ...
    { day:"wed", items:[ {run:"wed"}, isTestWeek
        ? mk(<BikeIcon size={13}/>,C.water,{t:"測驗前減壓",v:"30分·TSS15",x:`30分鐘 ${Z.Z1} 純輕鬆迴轉,迴轉數95-105,不做任何強度 — 明日FTP測驗,今天的任務是讓腿完全新鮮。跑步也建議只做輕鬆跑或休息。`},"wed-b")
    ...
    { day:"sat", items:[ {run:"sat"}, isTestWeek
        ? mk(<BikeIcon size={13}/>,C.power,{t:"測驗後恢復長騎",v:"150分·TSS105",x:`連續150分鐘 ${Z.Z2}下緣(${W(0.60)}-${W(0.70)}W),不做加強段;週四力竭測驗後神經疲勞需48-72小時,本週長騎只求時數不求強度。用新FTP的功率區間執行。`},"sat-b")
