...
function gen(phase, wi, rec){
  ...
  if (phase==="build1"){
    ...
    return { run:runOf(phase, Math.round((21+wi*2)*(rec?0.75:1)), rec), swim:{
      ...
      wed: wi===3 && !rec
        ? {t:"VO2max 天花板刺激", x:`熱身20分;主課 4x4分 @${W(1.05)}-${W(1.10)}W,組間4分完全恢復;緩和15分。取代本週Sweet Spot,目的是提高有氧天花板`, v:"67分", tss:76}
        : {t:"Z2+Sweet Spot", x:`${bw}分 @${W(0.62)}-${W(0.72)}W,中段2x15分 @${W(0.90)}-${W(0.93)}W(90-93%FTP),組間休5分 @${W(0.55)}-${W(0.65)}W(3:1);跑步間歇後4小時+`, v:`${bw}分`, tss:tss([[bw-30,0.65],[30,0.90]])},
      thu:{t:"FTP/TTE強化", x:`熱身15分;主課 ${rec?"2x12":(wi===1?"3x12":wi===2?"3x15":"3x18")} @${W(0.93)}-${W(0.95)}W;組間5分;緩和10分。目標是逐步延長接近FTP的累積時間,不追求騎爆`, v:rec?"59分":(wi===1?"71分":wi===2?"80分":"89分"), tss:rec?58:(wi===1?72:wi===2?82:91)},
  ...
  if (phase==="build2"){
    ...
    return { run:runOf(phase, Math.min(Math.round((24+wi*2)*(rec?0.75:1)),30), rec), swim:{
      ...
      wed: wi===2
        ? {t:"VO2max 天花板課", x:`熱身20分 @${W(0.60)}-${W(0.70)}W;主課 5x3分 @${W(1.10)}-${W(1.15)}W(110-115%FTP),組間休3分 @${W(0.45)}-${W(0.55)}W(1:1,完全恢復再出發);緩和15分。此為Build2的VO2max刺激,目的是推高有氧天花板 — 天花板升高後,下方Sweet Spot與比賽功率的耐受度會跟著提升。跑步間歇後4小時+`, v:"70分", tss:78}
      ...
      thu:{t:"FTP/TTE高峰", x:`熱身15分;主課 ${rec?"2x12":(wi===1?"2x25":wi===2?"3x18":"3x20")} @${W(0.93)}-${W(0.95)}W;組間6分;緩和10分。以延長TTE為主,品質下降就停`, v:rec?"61分":(wi===1?"87分":wi===2?"91分":"97分"), tss:rec?60:(wi===1?88:wi===2?92:98)},
