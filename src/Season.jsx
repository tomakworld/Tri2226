...
/* FTP:8/27實測20分平均210W(帶疲勞測),訓練基準採 203W
   下次重測 W15(10/12那週),巔峰期前最後校準 */
const FTP_TEST_WEEKS = { 15:"10/12當週(賽前最後校準)" };
...
function gen(phase, wi, rec){
  ...
  if (phase==="base"){
    ...
    return { run:runOf(phase, Math.round((16+wi*2)*(rec?0.8:1)), rec), swim:{
      ...
      wed:{t:"Z2+Tempo收尾", x:`${bw}分 @${W(0.62)}-${W(0.72)}W,最後15分拉至 @${W(0.78)}-${W(0.83)}W(連續不休);跑步間歇在前,間隔4小時+`, v:`${bw}分`, tss:tss([[bw-15,0.65],[15,0.83]])},
      ...
      sat:{t:"長騎有氧(功率為主)", x:`【鎖功率】全程維持 ${W(0.66)}-${W(0.69)}W,心率當護欄(上限145,超過且持續攀升就降到${W(0.60)}W完成時數);室內${r5(bs*0.85)}分/戶外${bs}分;每20分補水+電解質,每小時碳水60-70g;每45分變化一次(低迴轉65rpm/站姿/高迴轉110rpm);【續航力】最後30分補 2x8分 ${W(0.75)}-${W(0.80)}W`, v:`${bs}分`, tss:tss([[bs-30,0.66],[16,0.78],[14,0.6]])},
    }};
  }
  if (phase==="build1"){
    ...
    return { run:runOf(phase, Math.round((21+wi*2)*(rec?0.75:1)), rec), swim:{
      ...
      sun:{t:"開放水域✕配速", x:`連續${d3}m,抬頭定位,後段抓2:15/100m;緩和200m`, v:`${d3+200}m`},
    }, bike:{
      wed:{t:"Z2+Sweet Spot", x:`${bw}分 @${W(0.62)}-${W(0.72)}W,中段2x15分 @${W(0.90)}-${W(0.93)}W(90-93%FTP),組間休5分 @${W(0.55)}-${W(0.65)}W(3:1);跑步間歇後4小時+`, v:`${bw}分`, tss:tss([[bw-30,0.65],[30,0.90]])},
      thu:{t:"FTP強化", x:`熱身15分;主課 3x${bte}分 @${W(0.93)}-${W(0.95)}W(93-95%FTP,先從下緣${W(0.93)}W起);息5分(約3.5:1,短休息);緩和10分`, v:`${15+3*bte+15+10}分`, tss:tss([[15,0.6],[3*bte,0.92],[15,0.5],[10,0.55]])},
      sat:{t:"長距耐力", x:`【鎖功率】全程 @${W(0.66)}-${W(0.70)}W,心率護欄145;室內${r5(bs*0.85)}分/戶外${bs}分;每45分變化一次(低迴轉/站姿/高迴轉輪替),每小時起身1分;刻意練習空力姿勢維持;【續航力關鍵】最後45分補 3x10分 比賽功率${W(0.68)}-${W(0.72)}W,可依當日狀況彈性:守比賽功率(練續航力)或拉到Z3(練疲勞下的強度耐受),兩者交替使用`, v:`${bs}分`, tss:tss([[bs-45,0.66],[30,0.70],[15,0.6]])},
    }};
  }
  if (phase==="build2"){
    const r1=Math.max(4,Math.round((6+wi)*f)), r2=Math.max(3,Math.round((5+wi)*f));
    const d3=Math.max(2600,r50((3600+(wi-1)*250)*f));
    const bw=Math.max(70,r5((95+(wi-1)*5)*f)), bte=Math.min(20,Math.max(14,Math.round((18+(wi-1)*2)*f)));
    const bs=Math.max(195,r5((280+(wi-1)*15)*f));
    ...
    return { run:runOf(phase, Math.min(Math.round((24+wi*2)*(rec?0.75:1)),30), rec), swim:{
      ...
      fri:{t:"高強度閾值", x:`熱身400m;主課 ${r2}x300m 門檻 出發間隔2:35 (300m組:間隔7:45);緩和200m`, v:`${400+r2*300+200}m`},
      sun:{t:"長泳耐力", x:`連續${d3}m,模擬比賽節奏(泳池抓2:15)與補給;緩和200m`, v:`${d3+200}m`},
    }, bike:{
      wed: wi===2
        ? {t:"VO2max 天花板課", x:`熱身20分 @${W(0.60)}-${W(0.70)}W;主課 5x3分 @${W(1.10)}-${W(1.15)}W(110-115%FTP),組間休3分 @${W(0.45)}-${W(0.55)}W(1:1,完全恢復再出發);緩和15分。此為整期唯一一堂VO2max,目的是推高有氧天花板 — 天花板升高後,下方Sweet Spot與比賽功率的耐受度會跟著提升。跑步間歇後4小時+`, v:"70分", tss:78}
        : {t:"Z2+Sweet Spot", x:`${bw}分 @${W(0.62)}-${W(0.72)}W,中段2x18分 @${W(0.90)}-${W(0.93)}W(90-93%FTP),組間休6分 @${W(0.55)}-${W(0.65)}W(3:1);跑步間歇後4小時+`, v:`${bw}分`, tss:tss([[bw-36,0.65],[36,0.90]])},
      thu:{t:"FTP高峰", x:`熱身15分;主課 3x${bte}分 @${W(0.93)}-${W(0.95)}W(93-95%FTP);息5分(約3.5:1,短休息);緩和10分`, v:`${15+3*bte+15+10}分`, tss:tss([[15,0.6],[3*bte,0.93],[15,0.5],[10,0.55]])},
      sat:{t: brick?"長距+Brick":"長距耐力", x:`【本階段至少安排2次戶外】室內練不到坐墊耐受/實路操控/邊騎邊補給,【鎖功率】全程 @${W(0.67)}-${W(0.71)}W,心率護欄148;室內${r5(bs*0.85)}分/戶外${bs}分,每小時起身1分;【續航力關鍵】最後60分補 4x10分 比賽功率${W(0.68)}-${W(0.72)}W,組間5分Z2;比賽功率版練續航力、Z3版練疲勞下強度耐受,建議交替安排,越接近賽期越偏向比賽功率${brick?";下車再接20分超鐵配速跑(5:35-6:00)":""}`, v:`${bs}分`, tss:tss([[bs-60,0.67],[40,0.70],[20,0.6]])},
    }};
  }
  if (phase==="peak"){
    const T=[
      { run:runOf(phase,28,false), swim:{
        ...
        bike:{
          ...
          thu:{t:"FTP高峰", x:`熱身15分;主課 3x20分 @${W(0.93)}-${W(0.95)}W;息5分(約3.5:1,短休息);緩和10分`, v:"110分", tss:105},
      ...
      { key:true, run:runOf(phase,14,false), swim:{
        ...
        bike:{
          ...
          thu:{t:"FTP高峰", x:`熱身15分;主課 2x28分 @${W(0.92)}-${W(0.94)}W(長組略降);息8分;緩和10分`, v:"105分", tss:99},
          sat:{t:"🔑205km關鍵+Brick", x:`【比賽全模擬】鎖功率 ${W(0.68)}-${W(0.72)}W全程,心率護欄150;務必戶外(6小時室內練不到實路操控/風阻/補給動作),345-375分;完整演練比賽日補給(每小時70g碳水)與裝備;下車接40分超鐵配速跑 — 最重單日`, v:"6hr+40分", tss:330} } },
      { run:runOf(phase,20,false), swim:{
          ...
          sun:{t:"中距收量", x:"連續2400m 輕鬆連續游 2:25-2:35/100m;緩和200m", v:"2600m"} },
        bike:{
          wed:{t:"恢復迴轉", x:`45分 @${W(0.50)}-${W(0.62)}W`, v:"45分", tss:24},
          thu:{t:"FTP維持", x:`熱身15分;主課 3x12分 @${W(0.93)}-${W(0.95)}W;息4分(約4:1,短休息練有氧天花板與乳酸排除);緩和10分`, v:"75分", tss:71},
          sat:{t:"長騎收斂", x:`@${W(0.65)}-${W(0.70)}W 含30分比賽配速@${W(0.68)}-${W(0.72)}W;室內170分/戶外200分`, v:"3.3hr", tss:158} } },
    ];
  ...
  const T=[
    { run:{wed:{t:"神經喚醒",x:"6x200m 輕快",v:"6x200m"},thu:{t:"慢跑",x:"30分輕鬆",v:"30分"},fri:{t:"開合跑",x:"4km @226配速",v:"4km"},sat:{t:"慢跑",x:"20-30分+4x60m加速",v:"20-30分"},sun:{t:"中短長跑",x:"12km 輕鬆",v:"12km"}},
      swim:{ tue:{t:"減量有氧", x:"熱身300m;主課 4x100m EN2 出發間隔2:45 (游約2:25);緩和200m", v:"1000m"},
             fri:{t:"減量閾值", x:"熱身300m;主課 4x100m 門檻 出發間隔2:35;緩和200m", v:"1000m"},
             sun:{t:"輕鬆游", x:"連續1500m 輕鬆連續游 2:25-2:35/100m;緩和200m", v:"1700m"} },
      bike:{ wed:{t:"完全恢復", x:`30分 @${W(0.45)}-${W(0.55)}W`, v:"30分", tss:15},
             thu:{t:"神經敏銳", x:`熱身15分;主課 3x5分 @${W(0.95)}-${W(1.00)}W;息3分;緩和10分`, v:"40分", tss:40},
             sat:{t:"減量長騎", x:`@${W(0.62)}-${W(0.70)}W;室內75分/戶外90分`, v:"1.5hr", tss:63} } },
    { run:{wed:{t:"神經喚醒",x:"4x200m 輕快",v:"4x200m"},thu:{t:"慢跑",x:"20分輕鬆",v:"20分"},fri:{t:"開合跑",x:"3km @226配速",v:"3km"},sat:{t:"慢跑",x:"20分+短加速",v:"20分"},sun:{t:"中短長跑",x:"8km 輕鬆",v:"8km"}},
      swim:{ tue:{t:"賽前喚醒", x:"熱身300m;主課 4x50m 加速 息30秒;緩和200m", v:"700m"},
             fri:{t:"賽前開合", x:"熱身300m;主課 6x50m 抓1:05-1:07 息20秒;緩和200m", v:"700m"},
             sun:{t:"熟悉裝備", x:"連續1000m 輕鬆,比賽泳裝/防寒衣;緩和200m", v:"1200m"} },
      bike:{ wed:{t:"極輕量", x:`20分 @${W(0.45)}-${W(0.55)}W`, v:"20分", tss:10},
             thu:{t:"賽前開合", x:`20分 @${W(0.50)}-${W(0.62)}W,最後5分 @${W(0.78)}-${W(0.85)}W`, v:"20分", tss:14},
             sat:{t:"裝備確認", x:`@${W(0.62)}-${W(0.70)}W 45分,戶外比賽車+輪組,測補給品`, v:"45分", tss:32} } },
  ];
...
export default function SeasonPlan(){
  ...
  const rows = week.race ? [] : [
    ...
    { day:"tue", items:[ mk(<Dumbbell size={13}/>,C.iron,{t:isTestWeek?"重量訓練(測驗週減量)":"重量訓練",v:"",x:isTestWeek?"僅做動態熱身+2組x5輕負荷,總量減半;本週四FTP測驗,避免任何延遲性痠痛":STRENGTH[week.phase]},"tue-s"), mk(<Waves size={13}/>,C.water,week.swim.tue,"tue-sw") ] },
    { day:"wed", items:[ {run:"wed"}, isTestWeek
                      ...
                      if (it.run) {
                        ...
                        return (
                          <div key={ii} style={{ display:"flex", alignItems:"center", gap:6 }}>
                            <span style={{ display:"flex", alignItems:"center", gap:3, fontSize:11, color:RC[it.run], flexShrink:0, width:38 }}>
                              <Footprints size={12}/>跑
                            </span>
                            <input value={runNotes[key]||""} onChange={(e)=>onRun(week.n, it.run, e.target.value)} placeholder="填入跑步課表…"
